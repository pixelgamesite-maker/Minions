...
/* ── Fonts: Cormorant Garamond (editorial serif) + Space Grotesk (clean sans) ── */
const FONT_LINK =
...
function Label({ text }: { text: string }) {
  return (
    <div style={{ display:"flex", alignItems:"center", gap:"12px", marginBottom:"16px" }}>
      <div style={{ height:"1px", width:"28px", background:`linear-gradient(90deg,transparent,${gold}66)`, flexShrink:0 }} />
  ...
  return (
    <div onClick={handleClick} style={{ perspective:"1000px", cursor:locked?"not-allowed":flipped?"default":"pointer", animation:`cardIn 0.5s ease ${0.08*index}s both` }}>
      <div style={{ position:"relative", transformStyle:"preserve-3d", transition:"transform 0.6s cubic-bezier(0.23,1,0.32,1)", transform:flipped?"rotateY(180deg)":"rotateY(0)" }}>
        <div style={{ backfaceVisibility:"hidden", WebkitBackfaceVisibility:"hidden", position:flipped?"absolute":"relative", inset:0, background:bg, border:`1px solid ${borderCol}`, borderRadius:"12px", padding:"18px 14px", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:"8px", minHeight:"120px", opacity:locked?0.4:1, boxShadow:locked?"none":`0 0 20px ${gold}11` }}>
        ...
        <div style={{ backfaceVisibility:"hidden", WebkitBackfaceVisibility:"hidden", transform:"rotateY(180deg)", position:flipped?"relative":"absolute", inset:0, background:done?"linear-gradient(160deg,#121a10 0%,#0a0f08 100%)":bg, border:`1px solid ${done?`${gold}55`:`${gold}22`}`, borderRadius:"12px", padding:"14px 12px", minHeight:"120px", boxShadow:done?`0 0 20px ${gold}18`:"none" }}>
...
/* ── Gallery: big main image + dot nav ── */
function MinionGallery() {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:"0" }}>
      {/* Main image */}
      <div style={{ width:"100%", borderRadius:"16px 16px 0 0", overflow:"hidden", background:blueDeep, border:`1px solid ${blue}33`, borderBottom:"none", aspectRatio:"1/1" }}>
      ...
      {/* Thumbnail strip */}
      <div style={{ width:"100%", background:`linear-gradient(180deg,${blueDeep} 0%,#0a0c10 100%)`, border:`1px solid ${blue}22`, borderTop:`1px solid ${blue}33`, borderRadius:"0 0 16px 16px", padding:"16px 16px 14px", display:"flex", gap:"8px", overflowX:"auto" }}>
        {MINIONS.map((src, i) => (
          <button key={i} onClick={() => fade(i)} style={{ width:"52px", height:"52px", borderRadius:"8px", overflow:"hidden", border:i===cur?`2px solid ${blue}`:"2px solid transparent", flexShrink:0, padding:0, cursor:"pointer", opacity:i===cur?1:0.45, transition:"all 0.22s ease", background:"transparent" }}>
            <img src={src} alt="" style={{ width:"100%", height:"100%", objectFit:"cover", display:"block" }} />
          </button>
        ))}
      </div>
      {/* counter */}
      <p style={{ fontFamily:sans, fontSize:"0.56rem", letterSpacing:"0.24em", color:`${blue}88`, margin:"12px 0 0", textTransform:"uppercase" }}>
  ...
  return (
    <div style={{ borderBottom:`1px solid rgba(255,255,255,0.07)` }}>
      <button onClick={() => setOpen(o => !o)} style={{ width:"100%", background:"none", border:"none", cursor:"pointer", padding:"18px 0", display:"flex", alignItems:"center", justifyContent:"space-between", gap:"12px" }}>
        <span style={{ fontFamily:serif, fontSize:"1.05rem", fontWeight:600, color:open?goldLight:"rgba(255,255,255,0.78)", textAlign:"left", letterSpacing:"0.01em", transition:"color 0.2s" }}>{q}</span>
        <span style={{ color:gold, fontSize:"1.2rem", flexShrink:0, transition:"transform 0.28s", transform:open?"rotate(45deg)":"rotate(0)", lineHeight:1 }}>+</span>
      </button>
      {open && (
        <p style={{ fontFamily:sans, fontSize:"0.88rem", color:"rgba(255,255,255,0.42)", padding:"0 0 18px", margin:0, lineHeight:1.7 }}>{a}</p>
      )}
    </div>
  );
}
...
export default function Home() {
  ...
  const goldBtn = (label:string, onClick:()=>void, fullWidth=false) => (
    <button onClick={onClick} style={{
      ...
      padding:"15px 36px", cursor:"pointer", transition:"all 0.2s ease",
      boxShadow:`0 8px 32px ${gold}2e`, width:fullWidth?"100%":"auto",
    }}
      onMouseEnter={e=>{(e.currentTarget as HTMLButtonElement).style.background=goldLight;(e.currentTarget as HTMLButtonElement).style.transform="translateY(-1px)";}}
  ...
  const ghostBtn = (label:string, href:string) => (
    <a href={href} style={{
      ...
      padding:"15px 36px", display:"inline-block", transition:"all 0.2s ease",
    }}
      ...
      onMouseLeave={e=>{(e.currentTarget as HTMLAnchorElement).style.borderColor="rgba(255,255,255,0.14)";(e.currentTarget as HTMLAnchorElement).style.color="rgba(255,255,255,0.55)";}}>
      {label}
    </a>
  );
  ...
  return (
    <div style={{ background:"#050504", minHeight:"100vh", fontFamily:sans, color:"#fff", overflowX:"hidden" }}>
      ...
      <header style={{
        ...
        background:scrolled?"rgba(5,5,4,0.9)":"transparent",
        ...
        <a href="#home" style={{ display:"flex", alignItems:"center", gap:"10px" }}>
          ...
          <span style={{ fontFamily:serif, fontSize:"1.1rem", fontWeight:700, color:"#fff", letterSpacing:"0.1em" }}>MINIONS</span>
        </a>
        <nav style={{ display:"flex", alignItems:"center", gap:"28px" }}>
          {[["MinoList","#minolist"],["Mint","#mint"],["X","https://x.com/theminionxyz"]].map(([l,h])=>(
            <a key={l} href={h} target={l==="X"?"_blank":undefined} rel={l==="X"?"noopener noreferrer":undefined} style={{ fontFamily:sans, fontSize:"0.68rem", fontWeight:500, letterSpacing:"0.16em", textTransform:"uppercase", color:"rgba(255,255,255,0.42)", transition:"color 0.2s" }}
              onMouseEnter={e=>(e.currentTarget.style.color=goldLight)} onMouseLeave={e=>(e.currentTarget.style.color="rgba(255,255,255,0.42)")}>{l}</a>
          ))}
          <button onClick={()=>setModalOpen(true)} style={{
            fontFamily:sans, fontSize:"0.65rem", fontWeight:700, letterSpacing:"0.18em", textTransform:"uppercase",
            color:"#050504", background:gold, border:"none", borderRadius:"6px",
      ...
      <section id="home" style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"140px 24px 100px", textAlign:"center", position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ position:"absolute", top:"35%", left:"50%", transform:"translate(-50%,-50%)", width:"700px", height:"700px", borderRadius:"50%", background:`radial-gradient(circle,${blue}0a 0%,transparent 65%)`, pointerEvents:"none" }} />
        <div style={{ position:"absolute", bottom:"10%", right:"-10%", width:"400px", height:"400px", borderRadius:"50%", background:`radial-gradient(circle,${gold}06 0%,transparent 70%)`, pointerEvents:"none" }} />

        {/* pill */}
        <div style={{ animation:ready?"fadeUp 0.7s ease 0.05s both":"none", opacity:ready?undefined:0, marginBottom:"24px" }}>
          <span style={{ fontFamily:sans, fontSize:"0.58rem", fontWeight:600, letterSpacing:"0.28em", textTransform:"uppercase", color:gold, border:`1px solid ${gold}44`, borderRadius:"999px", padding:"6px 18px", display:"inline-block" }}>
            10,000 ON ETH
          </span>
        </div>
        ...
        {/* main title */}
        <h1 style={{
          fontFamily:serif, fontSize:"clamp(5rem,22vw,10rem)", fontWeight:700,
        ...
        {/* rule */}
        <div style={{ display:"flex", alignItems:"center", gap:"16px", width:"100%", maxWidth:"320px", margin:"24px auto",
        ...
        {/* subtext */}
        <p style={{
          fontFamily:serif, fontStyle:"italic", fontSize:"clamp(1rem,3vw,1.2rem)",
          color:"rgba(255,255,255,0.45)", margin:"0 0 44px", maxWidth:"400px", lineHeight:1.7,
        ...
        {/* stats grid */}
        <div style={{
          marginTop:"68px", display:"grid", gridTemplateColumns:"repeat(4,1fr)",
          ...
          {[["10,000","Supply"],["0.001 ETH","Mint Price"],["Ethereum","Chain"],["OpenSea","Launchpad"]].map(([v,l],i)=>(
            <div key={i} style={{ padding:"18px 20px", background:"rgba(255,255,255,0.018)", borderLeft:i>0?`1px solid ${gold}18`:"none", textAlign:"center" }}>
              <p style={{ margin:0, fontFamily:serif, fontSize:"1.1rem", fontWeight:700, color:goldLight }}>{v}</p>
              <p style={{ margin:"3px 0 0", fontFamily:sans, fontSize:"0.52rem", letterSpacing:"0.18em", textTransform:"uppercase", color:"rgba(255,255,255,0.28)", fontWeight:500 }}>{l}</p>
            </div>
          ))}
        </div>

        {/* scroll indicator */}
        <div style={{ position:"absolute", bottom:"38px", left:"50%", transform:"translateX(-50%)", display:"flex", flexDirection:"column", alignItems:"center", gap:"8px", opacity:0.4, animation:ready?"fadeUp 1.2s ease 1s both":"none" }}>
          <span style={{ fontFamily:sans, fontSize:"0.5rem", letterSpacing:"0.28em", textTransform:"uppercase", color:`${gold}` }}>Scroll</span>
          <div style={{ width:"1px", height:"32px", background:`linear-gradient(180deg,${gold},transparent)`, animation:"pulseGlow 2s ease infinite" }} />
        </div>
      </section>
      ...
      {/* ══════════ MEET THE MINIONS ══════════ */}
      <section style={{ padding:"100px 0", background:`linear-gradient(180deg,#050504 0%,${blueDeep}55 50%,#050504 100%)` }}>
        <div style={{ maxWidth:"680px", margin:"0 auto", padding:"0 24px" }}>
          <Label text="The Collection" />
          <h2 style={{ fontFamily:serif, fontSize:"clamp(2.2rem,7vw,3.6rem)", fontWeight:700, color:"#fff", margin:"0 0 12px", lineHeight:1.05 }}>Meet The Minions</h2>
          <p style={{ fontFamily:serif, fontStyle:"italic", fontSize:"1.05rem", color:"rgba(255,255,255,0.42)", margin:"0 0 48px", lineHeight:1.75, maxWidth:"480px" }}>
      ...
      <section style={{ padding:"100px 0" }}>
        <div style={{ maxWidth:"680px", margin:"0 auto", padding:"0 24px" }}>
          ...
          <h2 style={{ fontFamily:serif, fontSize:"clamp(2.2rem,7vw,3.6rem)", fontWeight:700, color:"#fff", margin:"0 0 12px", lineHeight:1.05 }}>Built Different</h2>
          <p style={{ fontFamily:serif, fontStyle:"italic", fontSize:"1.05rem", color:"rgba(255,255,255,0.42)", margin:"0 0 36px", lineHeight:1.75 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"8px", marginBottom:"28px" }}>
            {TRAITS.map(t => (
              <div key={t} style={{ display:"flex", alignItems:"center", gap:"10px", padding:"13px 16px", border:`1px solid ${blue}22`, borderRadius:"8px", background:`${blueDeep}44` }}>
                <div style={{ width:"5px", height:"5px", borderRadius:"50%", background:blue, flexShrink:0 }} />
                <span style={{ fontFamily:sans, fontSize:"0.84rem", fontWeight:500, color:"rgba(255,255,255,0.65)" }}>{t}</span>
              </div>
            ))}
          </div>

          <p style={{ fontFamily:serif, fontStyle:"italic", fontSize:"0.95rem", color:`${gold}99`, lineHeight:1.75, margin:0 }}>
      ...
      {/* ══════════ THE CLASSES ══════════ */}
      <section style={{ padding:"100px 0", background:`linear-gradient(180deg,#050504 0%,${blueDeep}44 60%,#050504 100%)` }}>
        <div style={{ maxWidth:"680px", margin:"0 auto", padding:"0 24px" }}>
          <Label text="The Types" />
          <h2 style={{ fontFamily:serif, fontSize:"clamp(2.2rem,7vw,3.6rem)", fontWeight:700, color:"#fff", margin:"0 0 40px", lineHeight:1.05 }}>Every Minion Has A Class</h2>
          <div>
            {CLASSES.map((c, i) => (
              <div key={c.name} style={{ padding:"22px 0", borderBottom:`1px solid rgba(255,255,255,0.07)`, display:"grid", gridTemplateColumns:"1fr auto", alignItems:"start", gap:"16px" }}>
                <div>
                  <p style={{ margin:"0 0 5px", fontFamily:serif, fontSize:"1.15rem", fontWeight:700, color:"#fff" }}>{c.name}</p>
                  <p style={{ margin:0, fontFamily:serif, fontStyle:"italic", fontSize:"0.95rem", color:"rgba(255,255,255,0.38)", lineHeight:1.6 }}>{c.desc}</p>
                </div>
                <span style={{ fontFamily:sans, fontSize:"0.52rem", letterSpacing:"0.22em", color:`${blue}66`, paddingTop:"4px", fontWeight:500 }}>
      ...
      {/* ══════════ JOIN THE MINOLIST ══════════ */}
      <section id="minolist" style={{ padding:"120px 0", background:`linear-gradient(180deg,#050504 0%,#0c0a06 50%,#050504 100%)` }}>
        <div style={{ maxWidth:"680px", margin:"0 auto", padding:"0 24px", textAlign:"center" }}>
          <Label text="Access" />
          <h2 style={{ fontFamily:serif, fontSize:"clamp(2.2rem,7vw,3.6rem)", fontWeight:700, color:"#fff", margin:"0 0 20px", lineHeight:1.05 }}>Join The MinoList</h2>
          <p style={{ fontFamily:serif, fontStyle:"italic", fontSize:"1.05rem", color:"rgba(255,255,255,0.42)", margin:"0 0 10px", lineHeight:1.8, maxWidth:"460px", marginLeft:"auto", marginRight:"auto" }}>
          ...
          <p style={{ fontFamily:sans, fontSize:"0.72rem", color:`${gold}88`, margin:"0 0 40px", letterSpacing:"0.08em", fontWeight:500 }}>
      ...
      {/* ══════════ MINT ══════════ */}
      <section style={{ padding:"100px 0" }}>
        <div id="mint" style={{ maxWidth:"680px", margin:"0 auto", padding:"0 24px" }}>
          <Label text="The Mint" />
          <h2 style={{ fontFamily:serif, fontSize:"clamp(2.2rem,7vw,3.6rem)", fontWeight:700, color:"#fff", margin:"0 0 12px", lineHeight:1.05 }}>One Phase. One Mint.</h2>
          <p style={{ fontFamily:serif, fontStyle:"italic", fontSize:"1.05rem", color:"rgba(255,255,255,0.42)", margin:"0 0 40px", lineHeight:1.7 }}>
            The MinoList is the mint.
          </p>

          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"1px", background:`${gold}18`, borderRadius:"10px", overflow:"hidden", marginBottom:"28px" }}>
            {[["10,000","Supply"],["0.001 ETH","Price"],["Ethereum","Chain"],["OpenSea","Launchpad"]].map(([v,l],i)=>(
              <div key={i} style={{ padding:"22px 20px", background:"#060503" }}>
                <p style={{ margin:0, fontFamily:serif, fontSize:"1.4rem", fontWeight:700, color:goldLight }}>{v}</p>
                <p style={{ margin:"4px 0 0", fontFamily:sans, fontSize:"0.54rem", letterSpacing:"0.2em", textTransform:"uppercase", color:"rgba(255,255,255,0.28)", fontWeight:500 }}>{l}</p>
              </div>
            ))}
          </div>

          <button disabled style={{
            width:"100%", fontFamily:sans, fontSize:"0.72rem", fontWeight:700, letterSpacing:"0.2em", textTransform:"uppercase",
            color:"rgba(255,255,255,0.2)", background:"rgba(255,255,255,0.03)",
            border:"1px solid rgba(255,255,255,0.07)", borderRadius:"6px", padding:"17px", cursor:"not-allowed",
          }}>
            MINT LOCKED
          </button>
          <p style={{ fontFamily:sans, fontSize:"0.66rem", color:`${gold}55`, textAlign:"center", margin:"12px 0 0", letterSpacing:"0.1em" }}>
      ...
      <section style={{ padding:"100px 0", background:"rgba(255,255,255,0.012)" }}>
        <div style={{ maxWidth:"680px", margin:"0 auto", padding:"0 24px" }}>
          <Label text="Reserve" />
          <h2 style={{ fontFamily:serif, fontSize:"clamp(2.2rem,7vw,3.6rem)", fontWeight:700, color:"#fff", margin:"0 0 16px", lineHeight:1.05 }}>The Mino Reserve</h2>
          <p style={{ fontFamily:serif, fontStyle:"italic", fontSize:"1.05rem", color:"rgba(255,255,255,0.42)", lineHeight:1.8, margin:0 }}>
      ...
      <section style={{ padding:"100px 0" }}>
        <div style={{ maxWidth:"680px", margin:"0 auto", padding:"0 24px" }}>
          <Label text="Token" />
          <h2 style={{ fontFamily:serif, fontSize:"clamp(3.5rem,14vw,7rem)", fontWeight:700, color:goldLight, margin:"0 0 16px", letterSpacing:"0.05em", lineHeight:0.9 }}>$MINO</h2>
          <p style={{ fontFamily:serif, fontStyle:"italic", fontSize:"1.05rem", color:"rgba(255,255,255,0.42)", lineHeight:1.8, margin:0 }}>
      ...
      <section style={{ padding:"100px 0", background:"rgba(255,255,255,0.012)" }}>
        <div style={{ maxWidth:"680px", margin:"0 auto", padding:"0 24px" }}>
          <Label text="Systems" />
          <h2 style={{ fontFamily:serif, fontSize:"clamp(2.2rem,7vw,3.6rem)", fontWeight:700, color:"#fff", margin:"0 0 40px", lineHeight:1.05 }}>The Systems</h2>
          <div>
            {SYSTEMS.map((s, i) => (
              <div key={s.name} style={{ padding:"22px 0", borderBottom:`1px solid rgba(255,255,255,0.07)`, display:"grid", gridTemplateColumns:"auto 1fr", alignItems:"start", gap:"20px" }}>
                <span style={{ fontFamily:sans, fontSize:"0.52rem", letterSpacing:"0.2em", color:`${gold}55`, paddingTop:"5px", fontWeight:600, flexShrink:0 }}>
                  {String(i+1).padStart(2,"0")}
                </span>
                <div>
                  <p style={{ margin:"0 0 5px", fontFamily:serif, fontSize:"1.12rem", fontWeight:700, color:goldLight }}>{s.name}</p>
                  <p style={{ margin:0, fontFamily:serif, fontStyle:"italic", fontSize:"0.95rem", color:"rgba(255,255,255,0.38)", lineHeight:1.65 }}>{s.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section style={{ padding:"100px 0" }}>
        <div style={{ maxWidth:"680px", margin:"0 auto", padding:"0 24px" }}>
          <Label text="The Plan" />
          <h2 style={{ fontFamily:serif, fontSize:"clamp(2.2rem,7vw,3.6rem)", fontWeight:700, color:"#fff", margin:"0 0 52px", lineHeight:1.05 }}>What Comes After Mint</h2>
          <div style={{ position:"relative", paddingLeft:"36px" }}>
            {/* vertical line */}
            <div style={{ position:"absolute", left:"7px", top:"4px", bottom:"4px", width:"1px", background:`linear-gradient(180deg,${gold}55,${gold}0a)` }} />
            <div style={{ display:"flex", flexDirection:"column", gap:"0" }}>
              {ROADMAP.map((r, i) => (
                <div key={r.phase} style={{ position:"relative", paddingBottom:i<ROADMAP.length-1?"32px":"0" }}>
                  {/* dot */}
                  <div style={{ position:"absolute", left:"-29px", top:"5px", width:"12px", height:"12px", borderRadius:"50%", border:`1.5px solid ${gold}`, background:"#050504" }} />
                  <p style={{ margin:"0 0 3px", fontFamily:sans, fontSize:"0.54rem", letterSpacing:"0.24em", textTransform:"uppercase", color:gold, fontWeight:600 }}>{r.phase}</p>
                  <p style={{ margin:"0 0 4px", fontFamily:serif, fontSize:"1.05rem", fontWeight:700, color:"rgba(255,255,255,0.88)" }}>{r.title}</p>
                  <p style={{ margin:0, fontFamily:serif, fontStyle:"italic", fontSize:"0.9rem", color:"rgba(255,255,255,0.35)", lineHeight:1.65 }}>{r.desc}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section style={{ padding:"100px 0", background:"rgba(255,255,255,0.012)" }}>
        <div style={{ maxWidth:"680px", margin:"0 auto", padding:"0 24px" }}>
          <Label text="FAQ" />
          <h2 style={{ fontFamily:serif, fontSize:"clamp(2.2rem,7vw,3.6rem)", fontWeight:700, color:"#fff", margin:"0 0 36px", lineHeight:1.05 }}>Questions</h2>
          <div>
      ...
      {/* ══════════ FOOTER ══════════ */}
      <footer style={{ padding:"72px 24px 48px", textAlign:"center" }}>
        <img src="/mini-logo.jpg" style={{ width:"42px", height:"42px", borderRadius:"8px", objectFit:"cover", marginBottom:"16px", opacity:0.9 }} alt="" />
        <h3 style={{ fontFamily:serif, fontSize:"1.5rem", fontWeight:700, color:goldLight, margin:"0 0 8px", letterSpacing:"0.1em" }}>MINIONS</h3>
        <p style={{ fontFamily:serif, fontStyle:"italic", fontSize:"0.9rem", color:"rgba(255,255,255,0.3)", margin:"0 0 28px", lineHeight:1.8 }}>
          Cute. Bold. Mainly for the cool ones.<br/>
          10,000 Minions on Ethereum. Powered by $MINO.
        </p>
        <div style={{ display:"flex", gap:"28px", justifyContent:"center", marginBottom:"40px" }}>
          {[["X","https://x.com/theminionxyz"],["OpenSea","#"],["MinoList","#minolist"],["Mint","#mint"]].map(([l,h])=>(
            <a key={l} href={h} style={{ fontFamily:sans, fontSize:"0.62rem", fontWeight:500, letterSpacing:"0.16em", textTransform:"uppercase", color:`${gold}66`, transition:"color 0.2s" }}
              onMouseEnter={e=>(e.currentTarget.style.color=goldLight)}
              onMouseLeave={e=>(e.currentTarget.style.color=`${gold}66`)}>{l}</a>
          ))}
        </div>
        <div style={{ width:"36px", height:"1px", background:`linear-gradient(90deg,transparent,${gold}44,transparent)`, margin:"0 auto 18px" }} />
        <p style={{ fontFamily:sans, fontSize:"0.52rem", letterSpacing:"0.3em", textTransform:"uppercase", color:`${gold}33`, fontWeight:500 }}>
